...
function ResearchTeam({apiKey}){
  ...
  async function sendMessage(){if(!input.trim()||!activeAgent||loading)return;const userMsg={role:"user",content:input.trim()};const updated=[...curConvo,userMsg];setConvs(c=>({...c,[activeAgent]:updated}));setInput("");setLoading(true);try{const res=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,system:agent.systemPrompt,messages:updated})});const data=await res.json();const txt=data.content?.find(b=>b.type==="text")?.text||"No response.";setConvs(c=>({...c,[activeAgent]:[...updated,{role:"assistant",content:txt}]}));}catch{setConvs(c=>({...c,[activeAgent]:[...updated,{role:"assistant",content:"\u26a0\ufe0f API fout."}]}));}setLoading(false);}
